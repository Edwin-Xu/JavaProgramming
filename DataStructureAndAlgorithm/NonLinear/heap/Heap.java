package DataStructureAndAlgorithm.NonLinear.heap;

/**
 * Created by XuTao on 2018/11/5 22:10
 * ···最小堆···
 * 《注意： 实际上并不需要用节点来真正构造一颗树，我们只是在数组中操作排序，调整好的数组就是一个堆的层遍历结果》
 * 不要高都没有搞懂就忙着写代码，先理解理解，构造树文明根本不用构造在调整，二是一边构造一边调整，以后先理解，吧作业写了，在来写代码，会简单得多
 * 而且，这个没有上机，主要是笔试，不要挂科
 *
 * 建树：
 * 一个一个添加（从数组中），添加到最后一个，树的最右最下方的那个，然后siftUp,从下往上调整就可以了 ，O（log2（n））
 *
 * 插入：
